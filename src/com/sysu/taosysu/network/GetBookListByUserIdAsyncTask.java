		...
		if (result != null) {
			try {
				...
				if (requestCode == 1) {
					...
					listener.onGetBookListByUserIdSuccess(bookList);
				} else {
					listener.onGetBookListByUserIdFail("获取书籍列表失败！");
				}
			} catch (JSONException e) {
				e.printStackTrace();
				listener.onGetBookListByUserIdFail("返回错误！");
			}
		} else {
			listener.onGetBookListByUserIdFail("网络连接错误！");
		}
	}
